...
router.put('/me', auth, async (req, res) => {
  try {
    ...
    const allowed = ['name', 'age', 'bio', 'gender', 'showMe', 'interests', 'relationshipGoal',
                     'photo', 'photos', 'height', 'distance'];
    const updates = {};
    ...
    if (updates.age !== undefined && updates.age !== null && updates.age !== '') {
      ...
      if (!Number.isFinite(updates.age) || updates.age < 18 || updates.age > 100) {
    ...
    if (updates.height !== undefined) {
      if (updates.height === null || updates.height === '') {
        ...
        if (!Number.isFinite(updates.height) || updates.height < 100 || updates.height > 250) {
    ...
    if (updates.distance !== undefined) {
      ...
      if (!Number.isFinite(updates.distance) || updates.distance < 1 || updates.distance > 500) {
        return res.status(400).json({ error: 'distance must be between 1 and 500 km' });
      }
    }
    ...
    if (updates.showMe !== undefined) {
      ...
      if (String(updates.showMe || '').trim() && !canonical) {
        return res.status(400).json({ error: 'showMe must be Man, Woman, Other, or All' });
      }
      updates.showMe = canonical;
    }

    if (updates.showMe || updates.gender !== undefined) {
      const current = await User.findById(req.userId).select('gender showMe').lean();
      if (!current) return res.status(404).json({ error: 'User not found' });

      if (updates.showMe) {
        updates['discoveryPrefs.gender'] = updates.showMe;
        updates['discoveryPrefs.updatedAt'] = new Date();
      } else if (updates.gender !== undefined && req.body.showMe === undefined) {
        const nextShowMe = followGenderChange(current.gender, current.showMe, updates.gender);
    ...
    const protectedFields = ['tokenBalance', 'lastSpinDate', 'activeDeviceId',
                             'chatSessionStartedAt', 'chatSessionExpiresAt',
                             'isVerified', 'isOnline', 'email', 'location', 'userId', '_id',
                             'publicId'];
    ...
    const publicFieldsChanged = [
      'name', 'bio', 'photo', 'photos', 'age', 'gender', 'height',
      'interests', 'relationshipGoal',
...
router.put('/location', auth, async (req, res) => {
  try {
    ...
    if (latitude === 0 && longitude === 0) {
      return res.status(400).json({ error: 'Could not read a valid GPS fix. Try again.' });
    }
...
router.get('/profile/:userId', auth, async (req, res) => {
  try {
    ...
    if (userId === req.userId.toString()) {
      return res.status(400).json({ error: 'Use GET /me to view your own profile' });
    }
    ...
    const safe = await applyBlockPrivacy(req.userId, user.toObject ? user.toObject() : user);
...
router.get('/search-by-id', auth, async (req, res) => {
  try {
    const publicId = String(req.query.publicId || '').toUpperCase().trim();
    ...
    if (!/^[A-Z]{4}[0-9]{4}$/.test(publicId)) {
      return res.status(400).json({ error: 'Invalid publicId format. Expected: ABCD1234' });
    }
    ...
    const user = await User.findOne({
      ...
      _id: { $ne: req.userId },
      isVerified: true,
    }).select('publicId name age bio photo photos gender interests height relationshipGoal isOnline lastSeen location subscriptionPlan subscriptionExpiresAt');
    ...
    const iLiked =
      ...
      (friendship?.status === 'pending_like' && friendship.initiatedBy.equals(req.userId));
    const theyLiked =
      areFriends ||
      friendship?.status === 'mutual_match' ||
      (friendship?.status === 'pending_like' && friendship.initiatedBy.equals(user._id));
...
function rememberDiscoveryPrefs(userId, current, next) {
  ...
  const radiusKm = Number.isFinite(next.radiusKm) && next.radiusKm > 0
  ...
  const $set = {
    discoveryPrefs: { gender, radiusKm, activeWithinMinutes, updatedAt: new Date() },
  };
...
router.get('/nearby', auth, async (req, res) => {
  try {
    ...
    const me = await User.findById(req.userId).select('location gender showMe discoveryPrefs').lean();
    if (!me) return res.status(404).json({ error: 'User not found' });

    if (!hasRealLocation(me.location?.coordinates)) {
      return res.status(400).json({ error: 'Update your location to see nearby people.' });
    }
